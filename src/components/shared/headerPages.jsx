import React from "react";
import Image from "next/image";

export const HeaderPages = ({ title, logo, image }) => {
  return (
    <div className='flex h-screen md:h-auto w-full relative'>
      <Image
        src={image.imageMobile.url}
        width={394}
        height={850}
        alt='Hero Image'
        className='flex md:hidden relative w-full'
      />
      <Image
        src={image.imageDesktop.url}
        width={1920}
        height={850}
        alt='Hero Image'
        priority={true}
        className='hidden md:flex relative w-full md:object-top md:object-contain md:top-0 lg:object-cover'
      />
      <div className='flex absolute z-50 items-center justify-center text-white h-full w-full'>
        {title ? (
          <h1 className='uppercase font-mont font-bold text-white text-opacity-70 leading-[70px] text-5xl md:text-3xl lg:text-5xl pl-[15px] pr-[30px]'>
            {title}
          </h1>
        ) : logo ? (
          <Image
            src={logo}
            width={800}
            height={200}
            alt='Logo'
            className='flex md:h-full md:w-[60vw] w-[80vw]'
          />
        ) : null}
      </div>
    </div>
  );
};
